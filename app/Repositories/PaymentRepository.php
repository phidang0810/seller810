...
Class PaymentRepository
{
    ...
    {
        $products = PaymentDetail::selectRaw('products.name, products.main_cate, products.barcode_text, products.photo, products.category_ids, payments.platform_id, SUM(payment_detail.quantity) as quantity, SUM(payment_detail.total_price) as total_price, SUM(payment_detail.total_price - (payment_detail.import_price*payment_detail.quantity) - payment_detail.discount_amount)  as profit, DATE(payment_detail.created_at) as created_at, COUNT(payments.cart_id) total_cart')
            ->join('products' ,'products.id', '=', 'payment_detail.product_id')
