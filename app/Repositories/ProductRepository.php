...
Class ProductRepository
{
    ...
    public function dataTable($request)
    {
        ...
        $dataTable = DataTables::eloquent($products)
            ...
            ->addColumn('action', function ($product) {
                ...
                if (Auth::user()->email == 'phidangmtv@gmail.com' || Auth::user()->email == 'van@gmail.com') {
    ...
    public function createOrUpdate($data, $id = null)
    {
        ...
        if (isset($data['photo'])) {
            ...
            $model->thumb = $upload->resizeTo(500);
        }
    ...
    {
        $model = Product::select('photo', 'thumb', 'name', 'id', 'slug');
        if (key_exists('is_home', $search)) {
            $model->where('is_home', $search['is_home']);
        }

        if (key_exists('is_hot', $search)) {
